...
root.title("YouTube Video Downloader | Smart Mirror Project")
...
# print the response object from the api request
print("The response object from the api request : " + str(data))

# The api request was not working that's why we used a data_test object to work on it
...
print("The video name that we are working on is: %s" % video_name, "The video link is: %s" % video_link)
...
def progress_function():
    ...
    while p <= 100:
        try:
            # We need to have a path directory otherwise the we will get FileNotFoundError, (In this code the error will not show!!!)
...
def Download():
    ...
    if (youtube_link != ""):
        ...
        if (youtube_video_name == None):
            ...
            youtube_video_name = youtube_video_name[:30]

            print("The youtube video name : " + youtube_video_name)

        size = videoStream.filesize

        # Just checking the size of the video file
        print("File Size in bytes: " + str(size))

        # To run multiple function at once you need to use library threading
        if __name__ == '__main__':
            ...
            Thread(target=IsDownloaded).start()
...
# IsDownloaded() function to verify if the youtube downloaded or not
def IsDownloaded():
    res = 0  # false
    while (res == 0):
        # check if progressbar['value'] equal to 100
        if (progressbar['value'] == 100):
            # Displaying the message
            messagebox.showinfo("SUCCESSFULLY", "DOWNLOADED AND SAVED IN\n" + download_Folder)
            res = 1  # return true and stop the while loop
